...
- **PersonalNamespaces may be empty**: Some IMAP servers return empty `PersonalNamespaces`. `GetFoldersAsync` with a manually constructed `FolderNamespace` fails on these servers. `SyncFoldersCoreAsync` falls back to `client.GetFolder("")` + recursive `GetSubfoldersAsync` to enumerate all folders from the root
